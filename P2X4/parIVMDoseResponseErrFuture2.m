...
%         tic
        F=parPulseDoseResponseFuture2(x,model);
...
%         toc
    ...
    function err=evalFutures(F)
        ...
        tauoff=zeros(6,1);
        for i=1:6
            [j,Ipeak,tmax,Ibase,toff] = fetchNext(F);
            Imax(j)=Ipeak;
            Tmax(j)=Tpeak;
            tauoff(j)=toff;
        end
        if sum(isnan([Imax; tauoff]))~=0
            ...
            if sum(Tmax>2)~=0
                err=sum(((Imax-Icurve)./(r2*semIcurve)).^2)+sum(((tauoff-taucurve)./(r2*semtaucurve)).^2)+sum(((2-Tmax)./(r2*0.1)).^2);
            else
