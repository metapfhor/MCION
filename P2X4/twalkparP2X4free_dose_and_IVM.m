...
p = struct('globalStruct',glblStruct,'logPrior', @(x) dilation(x), 'logLikelihood',{{@(x) 100*err4(x),@(x) 100*err2(x),@(x) 100*err3(x),@(x) 100*err5(x),@(x) 1000*err7(x) }},'bounds',bounds);
...
save(['twalk_P2X4free_preStim_and_IVM' num2str(Niter) '_steps@' datestr(now,30) '.mat' ],'x0','xp0','xxp','lt','acc','fparam','baseParameters','parami','iparam','parami')
