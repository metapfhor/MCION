function [EC,Imax]= doseEC(tprestim,model)
...
%         cf(1);
%         semilogx(ATP./1e-6,Imax);
% %         title([num2str(A(i)/1e-6) ' uM ATP ' num2str(tprestim) ' s of pre-stimulation']);
%         drawnow
    rescon=real(ec502(ATP./1e-6,Imax));
    EC=rescon(3);
    Imax=Imax(end);
end
