...
%         toc
    ...
    function err=evalFutures(F)
        ...
        if sum(isnan([Imax; tauoff]))~=0
            ...
            if  Imax(2,6)<Imax(1,6) || Imax(2,4)<Imax(1,4) || Imax(2,3)<Imax(1,3)
                err=Inf;
                return;
            end
...
% %             activation time (control)
            err2=err2+sum(((Tmax(:,1)-1.15)./(r2*0.1)).^2);
%            activation time (1 um IVM) only the first point
            err2=err2+sum(((Tmax(1,3).'-[0.8352])./(r2*[0.1])).^2);
			%activation time difference(1 um IVM)
            err2=err2+sum((((Tmax(1,3)-Tmax(2,3)).'-(0.8352-1.206))./(r2*[0.1])).^2);
			
            %we dont want any activations to finish after 1.2 seconds
            if sum(Tmax(:)>1.3)~=0
                err2=err2+sum((Tmax(Tmax(:)>1.3)-1.3)./(r2*0.1).^2);
            end
%            %activation time difference(3 um IVM)
            err2=err2+sum((((Tmax(1,4)-Tmax(2,4)).'-(0.8352-1.206))./(r2*[0.1])).^2);
            %activation time difference (10 um IVM)
            err2=err2+sum((((Tmax(1,6)-Tmax(2,6)).'-(0.8352-1.206))./(r2*[0.1])).^2);
            ...
            err2=err2+((Mdes(1,3)-Mdes(2,3)-0.1410)/(r2*0.05))^2;
