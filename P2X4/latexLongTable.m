...
% obtain cell array for the table data and labels
colLabelExist = isfield(input,'tableColLabel');

% process table datatype
if isa(input.data,'table')
    if colLabelExist
        input.tableColLabels = [input.tableColLabel,input.data.Properties.RowNames(:).']';
    else
        input.tableColLabels = input.data.Properties.RowNames';
    end
    input.tableRowLabels = input.data.Properties.VariableNames';
...
for i=1:size(input.data,2)
    if isnumeric(table2array(input.data(1,i)))
        C(1+rowLabelsExist:end,colLabelsExist+i) = num2cell(table2array(input.data(:,i))).';
    else
        C(1+rowLabelsExist:end,colLabelsExist+i) = char2cell(table2array(input.data(:,i))).';
...
if rowLabelsExist
    C(1,1+colLabelsExist:end)=input.tableRowLabels.';
end

if isa(input.data,'table') && colLabelExist
C(1:end,1)=input.tableColLabels;
elseif colLabelsExist
    
    C(1+rowLabelsExist:end,1)=input.tableColLabels;
end
...
for i=1:size(input.data,1)
    dataFormatArray(1+colLabelsExist:end,rowLabelsExist+i) = tmp;
end
% transpose table (if this switched on)
if input.transposeTable
    C = C';
...
% latex = {'\centering';header;['\caption{',input.tableCaption,'}']};
latex = {'\centering';header};
% generate table
for i=1:size(C,1)
    if input.tableBorders
        latex(end+1) = {hLine};
...
%     latex(end+1) = {[rowStr,' \\']};
    latex(end+1) = {[rowStr,' \tabularnewline']};
    if i==1
        latex(end+1) = {'\endhead'};
    end
end
...
footer = {'\end{longtable}'; ...
    ['\label{table:',input.tableLabel,'}'];};

 footer = {['\caption{',input.tableCaption,'}'];'\end{longtable}'};
