function h = plotProlongedPanelCurr(experiment,model,row)
    fs=12;
	model.setParams([]);
...
%         end
    ...
    for i=1:length(ton)
        ...
        [y]=y_to_norm(0,0);
        if i==1
              annotation('TextBox',[xmax(2),y(1)-0.004,1,0],'String',[num2str(experiment.globals.A/1e-6) ' \muM ATP'],'VerticalAlignment','baseline','HorizontalAlignment','left','LineStyle','none','FontSize',fs);
        end
...
%         
%         [x]=x_to_norm(experiment.conditions{1},experiment.conditions{2});
%         [y]=y_to_norm(0,0);
% 
%         annotation('Rectangle',[x(1),y(1)+d,x(2)-x(1),h],'FaceColor',[0,0,0],'FaceAlpha',1,'LineStyle','none');
%         annotation('TextBox',[x(1),y(1)+h,x(2)-x(1),0],'String',[num2str(experiment.globals.A/1e-6) ' \muM ATP'],'VerticalAlignment','baseline','HorizontalAlignment','left','LineStyle','none','FontSize',fs);
% % 
     ...
     [x]=x_to_norm(experiment.globals.IVMon,min(experiment.globals.IVMoff,experiment.conditions{3}));
        ...
        annotation('TextBox',[xmax(2),y(1)-0.008+2*d+h,x(2)-x(1),0],'String',[num2str(experiment.globals.J/1e-6) ' \muM IVM'],'VerticalAlignment','baseline','HorizontalAlignment','left','LineStyle','none','FontSize',fs,'Color',civm);
    else
%         
