...
global baseParameters fparam parami nbin Acell;
...
N=4e2;
% Run the PT
[X, m, R, theta, beta, stats, sse, XA] = cell_adaptive_pt(@(x) -(1.0*err5(x)+1.0*err3(x)+3.0*err2(x)), param, N,Ncell);
% [X, m, R, theta, beta, stats, sse, XA] = cell_adaptive_pt(@(x,l) -err2(x,l), param, N,Ncell);

% cf(1);chainConvergence(X,beta,sse);


% cf(9);plotDensity(sse(sse<2*10^5));

goodSamples=cullChain(X,sse,1.1*10^4); %if it runs into a problem here, just reevaluate this part.
...
cf(4);plotNaiveResponse(pulse3IVM,P2X4);

cf(5);plotNaiveResponse(pulse10IVM,P2X4);

cf(6);plotNaiveResponse(prol3IVM,P2X4);

cf(7);plotNaiveResponse(pulse1IVM,P2X4);

% cf(3);plotNaiveResponse(prol2noIVM,P2X4);
% 
% cf(2);plotNaiveResponse(pulse0IVM,P2X4);
