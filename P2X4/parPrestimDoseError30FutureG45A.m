...
I=zeros(7,3);
    function F=sseFutures(x)
        ...
        F(1)=parfeval(@globalSetFeval,1,g,@ATPDoseResponseG45A,x,m{1});
        F(2)=parfeval(@globalSetFeval,1,g,@ATPDoseResponse30G45A,x,m{2});
...
%         err=(sum(((I0p-I0).^2)./(2*v0.^2))+sum(((I3p-I3).^2)./(2*v3.^2)));
          err=sum([ ((Imax30 - 4.18e+3)^2)/(2*(0.3e+2)^2) ((EC30-0.2e-6)^2)/(2*(0.5e-7)^2) ((ECcontrol-1.6e-6)^2)/(2*(0.1e-6)^2)  ((Imaxcontrol - 3.23e+3)^2)/(2*(0.3e+2)^2)]);
    end
