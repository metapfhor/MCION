...
[err7] = normDataErrorOmar(prol3IVMp,factorP2X4coop);

etot = @(x) 1000*err1(x)+err2(x)+err3(x)+err4(x)+err5(x)+err6(x);

emed = @(x) err2(x)+err3(x)+err5(x)+err7(x);
...
p = struct('logPrior', @(x) intern(x) & coop(x), 'logLikelihood',@(x) -(emed(x)),'bounds',bounds);
...
% 
% cf(21);plotNaiveResponse(prol3IVMp,factorP2X4coop);
%  cf(20);chainPrestimDoseResponse(x0,factorP2X4coop);
 

save(['twalk_factorP2X4coop_IVMonly' num2str(Niter) '_steps@' datestr(now,30) '.mat' ],'x0','xp0','xxp','lt','acc','fparam','baseParameters','parami','iparam','parami')
