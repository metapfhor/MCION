function h = plotProlongedPanel(experiment,model,row)
    fs=12;
	model.setParams([]);
...
%     
	...
	plot(T,[model.getOpen(Y), model.getDilated(Y)],'LineWidth',1.5);
        axis tight
%     plot(T,[Y(:,1) Y(:,13) Y(:,15) Y(:,17) Y(:,7)+Y(:,8)],'LineWidth',2); %Y(:,9)+Y(:,10) Y(:,11)+Y(:,12) Y(:,13) Y(:,15) Y(:,17)],'LineWidth',2);
% 	
%     plot(T,[Y(:,9)+Y(:,10) Y(:,11)+Y(:,12) Y(:,3) Y(:,5) Y(:,6)],'LineWidth',2);
    if row==1
        legend('Open','Dilated','Location','NorthEast');
    end
