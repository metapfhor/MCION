...
paramNames=['k1','k2','k3','k4','k5','k6','L1','L2','L3','H1','H2','H3','H4','H5','H6','H7','rca','kca','koff','C','g12','g34','E12','E34','A','V'];
m=new model('calP2X7');
...
m.calLinkStates=function(s1,s2,kfor,kback){
	...
	function replaceKinetics(k) {
		...
		if (out instanceof algebra) {
			out=out.replaceString('k1','k1*rca');
			out=out.replaceString('k3','k3*rca');
			out=out.replaceString('k5','k5*rca');
			out=out.replaceString('k2','k2*rca');
			out=out.replaceString('k4','k4*rca');
			out=out.replaceString('k6','k6*rca');
		}else{
			out=out.replace(/k1/g,'k1*rca');
			out=out.replace(/k3/g,'k3*rca');
			out=out.replace(/k5/g,'k5*rca');
			out=out.replace(/k2/g,'k2*rca');
			out=out.replace(/k4/g,'k4*rca');
			out=out.replace(/k6/g,'k6*rca');
		}
		return out;
	}
