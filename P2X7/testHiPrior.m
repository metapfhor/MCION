...
%No Cal, High ATP
[err,Ncell] = cellDataError(exp32,fullP2X7);
...
[X, m, R, theta, beta, stats, sse, XA] = cell_adaptive_pt_prior(@(x,l) -err(x,l), param, Niter, Ncell);

save(['calBindingHi' num2str(Niter) '_steps@' num2str(L) '_levels_' datestr(now,30) '.mat' ],'X','beta','sse','R','fparam','baseParameters','parami','XA')
...
goodSamples=cullChain(X,sse,0.5*10^8);
...
% cf(7);plotNaiveResponse(exp32,fullP2X7);