...
%No Cal, High ATP
[err,Ncell] = cellDataError(exp10,fullP2X7);
...
save(['calBindingMed' num2str(Niter) '_steps@' num2str(L) '_levels_' datestr(now,30) '.mat' ],'X','beta','sse','R','fparam','baseParameters','parami','XA')

%cf(9);plotDensity(sse(sse<4.5*10^5));

goodSamples=cullChain(X,sse,3*10^2);
...
%cf(4);plot(1:Niter,squeeze(XA));
 ...
 cf(5);plotChainMed(fullP2X7,goodSamples,10);
 %cf(6);plotChainMed(calP2X7,goodSamples,10);
 
 %cf(7);plotNaiveResponse(exp10,fullP2X7);