...
exp1.conditions={tstart,tend,ttot};
exp1.globals=struct('ATP',0.1*10^-6,'A',0.1*10^-6,'V',-60*10^-3,'C',0);
exp1.simple.conditions={5,125,245};
exp1.simple.globals=struct('ATP',0.1*10^-6,'A',0.1*10^-6,'V',-60*10^-3,'C',0);
...
rec1(t>tstart&t<=tend)=-5.9*(1-exp(-(t(t>tstart&t<=tend)-tstart)/tauact));
rec1(t>tend)=-rec1(t==tend)*(1-exp(-(t(t>tend)-tend)/taude))+rec1(t==tend);
%%quick hack to get amplitudes in the right range
% rec1(t>tstart&t<=tend)=-15.9*(1-exp(-(t(t>tstart&t<=tend)-tstart)/tauact));
% rec1(t>tend)=-rec1(t==tend)*(1-exp(-(t(t>tend)-tend)/taude))+rec1(t==tend);
exp1.data=[t,rec1];
...
exp3.conditions={tstart,tend,ttot};
exp3.globals=struct('ATP',0.3*10^-6,'A',0.3*10^-6,'V',-60*10^-3,'C',0);

tauact=12;
taude=40;
rec3=zeros(length(t),1);
rec3(t>tstart&t<=tend)=-36.5*(1-exp(-(t(t>tstart&t<=tend)-tstart)/tauact));
rec3(t>tend)=-rec3(t==tend)*(1-exp(-(t(t>tend)-tend)/taude))+rec3(t==tend);
...
rec10(ttot>=5&ttot<=6)=-350*(1-decay(1,0.3,1,dtfast));
rec10(ttot>6&ttot<=66)=150*(1-exp(-t/4))-1400*(1-exp(-t/80))-350;
...
% tslow=tstart+tfast;
% ttot=tend;
% exp10.conditions={tstart,tend,ttot};
% t=[0,(tstart:dtfast:tslow-dtfast),(tslow:dt:ttot)].';
% 
% rec10=zeros(length(t),1);
% rec10(t>=tstart&t<=tslow)=-350*(1-decay(1,0.3,tfast,dtfast));
% tp=t(t>tslow&t<=tend)-tslow;
% rec10(t>tslow&t<=tend)=140*(1-exp(-tp/4))-1400*(1-exp(-tp/80))-350;
% 
% exp10.data=[t,rec10];
...
rec32(ttot>=5&ttot<=6)=-1300*(1-decay(1,0.2,1,dtfast));
rec32(ttot>6&ttot<=46)=470*(1-exp(-t/2))-1800*(1-exp(-t/6))-1300;
...
tslow=tstart+tfast;
ttot=2*thold+tstart;
exp100.conditions={tstart,tend,ttot};
t=[0,(tstart:dtfast:tslow-dtfast),(tslow:dt:ttot)].';

rec100=zeros(length(t),1);
rec100(t>=tstart&t<=tslow)=-2500*(1-decay(1,0.1,tfast,dtfast));
tp=t(t>tslow&t<=tend)-tslow;
rec100(t>tslow&t<=tend)=140*(1-exp(-tp))-1000*(1-exp(-tp/8))-2500;
tp=t(t>tend)-tend;
rec100(t>tend)=rec100(t==tend)*(exp(-tp/taude));

exp100.data=[t,rec100];

%NO deactivation for exp100
...
%cf(1);plotPanelsResponse(fullerP2X7);