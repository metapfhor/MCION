function ode = modelPulseODEpotent1P2X4revcoop(ton,toff)
global k1 k2 k3 k4 k5 k6 L1 L2 L3 L4 L5 L6 L7 H1 H2 H3 H4 g1 g2 E1 E2 delta epsilon mu A J V n1 n2 n3 v1 v2 v3 r1 r2 r3 a1 alpha beta gamma A0 J0 IVMon IVMoff;
	ATP=@(t) 0;
	for i=1:length(ton)
	...
	function dy=model(t,y)
		setAuxiliarypotent1P2X4revcoop(y);
		dy=zeros(21,1);
		A=ATP(t);
		J=IVM(t);
		...
		dy(3)=v1*k1*y(4)+L2*J^n1/(J^n1+delta^n1)*y(1)+L7*y(5)-(r1*k2*A+L1)*y(3);%C3
		dy(4)=r1*k2*A*y(3)+v1*k3*y(11)+alpha*L2*J^n1/(J^n1+delta^n1)*y(2)+L7*y(6)-(v1*k1+r1*k4*A+L1)*y(4);%C4
		dy(5)=v2*k1*y(6)+L7*y(7)-(r2*k2*A+L7)*y(5);%C5
		dy(6)=r2*k2*A*y(5)+v2*k3*y(13)+L7*y(8)-(v2*k1+r2*k4*A+L7)*y(6);%C6
		...
		dy(11)=r1*k4*A*y(4)+v1*k5*y(12)+beta*L2*J^n1/(J^n1+delta^n1)*y(9)+L7*y(13)-(v1*k3+r1*k6*A+L1)*y(11);%Q3
		dy(12)=r1*k6*A*y(11)+gamma*L2*J^n1/(J^n1+delta^n1)*y(10)+L3*y(14)-(v1*k5+L1+L4*J^n2/(J^n2+epsilon^n2))*y(12);%Q4
		dy(13)=r2*k4*A*y(6)+v2*k5*y(14)+L7*y(15)-(v2*k3+r2*k6*A+L7)*y(13);%Q5
