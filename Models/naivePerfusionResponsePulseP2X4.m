function [Tout,Isoln,Ysoln] = naivePerfusionResponsePulseP2X4(ton,toff,Ttot,T)
global A J p0 err IVMon IVMoff I Y imin imax t0 Acell Tsoln;
    ...
    ind=find(ton<IVMon);
    ion=ind(end); %Sets index value for when IVM turns on.
    ...
    for i=1:ion-1
        ...
        indPre=find(T>IVMon&T<=ton(ion+1));
		indOn=find(T>ton(ion+1)&T<=toff(ion+1));
        indOff=find(T>toff(ion+1)&T<=ton(ion+2));
