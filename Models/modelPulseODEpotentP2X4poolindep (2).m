function ode = modelPulseODEpotentP2X4poolindep(ton,toff)
global k1 k2 k3 k4 k5 k6 kd1 ku1 kd2 ku2 kd3 ku3 L1 L2 L3 L4 L5 L6 H1 H2 H3 H4 g1 g2 E1 E2 delta epsilon mu A J V n1 n2 n3 v1 v2 v3 r1 r2 r3 w1 w2 w3 s1 s2 s3 A0 J0 IVMon IVMoff;
	ATP=@(t) 0;
	for i=1:length(ton)
	...
	function dy=model(t,y)
		...
		dy(9)=w1*k1*y(10)+kd1*J*y(3)+L3*y(11)-(s1*k2*A+ku1+L4*J^4/(J^4+epsilon^4))*y(9);%CD3
		dy(10)=s1*k2*A*y(9)+w1*k3*y(23)+kd1*J*y(4)+L3*y(12)-(w1*k1+s1*k4*A+ku1+rho1*L4*J^4/(J^4+epsilon^4))*y(10);%CD4
		dy(11)=w2*k1*y(12)+kd2*J*y(5)+L4*J^4/(J^4+epsilon^4)*y(9)+L5*y(13)-(s2*k2*A+ku2+L3+L6*J^4/(J^4+mu^4))*y(11);%CD5
		dy(12)=s2*k2*A*y(11)+w2*k3*y(25)+kd2*J*y(6)+rho1*L4*J^4/(J^4+epsilon^4)*y(10)+L5*y(14)-(w2*k1+s2*k4*A+ku2+L3+L6*J^4/(J^4+mu^4))*y(12);%CD6
		dy(13)=w3*k1*y(14)+kd3*J*y(7)+L6*J^4/(J^4+mu^4)*y(11)-(s3*k2*A+ku3+L5)*y(13);%CD7
		dy(14)=s3*k2*A*y(13)+w3*k3*y(27)+kd3*J*y(8)+L6*J^4/(J^4+mu^4)*y(12)-(w3*k1+s3*k4*A+ku3+L5)*y(14);%CD8
		...
		dy(23)=s1*k4*A*y(10)+w1*k5*y(24)+kd1*J*y(17)+L3*y(25)-(w1*k3+s1*k6*A+ku1+rho2*L4*J^4/(J^4+epsilon^4))*y(23);%QD3
		dy(24)=s1*k6*A*y(23)+kd1*J*y(18)+L3*y(26)-(w1*k5+ku1+rho3*L4*J^4/(J^4+epsilon^4))*y(24);%QD4
		dy(25)=s2*k4*A*y(12)+w2*k5*y(26)+kd2*J*y(19)+rho2*L4*J^4/(J^4+epsilon^4)*y(23)+L5*y(27)-(w2*k3+s2*k6*A+ku2+L3+L6*J^4/(J^4+mu^4))*y(25);%QD5
		dy(26)=s2*k6*A*y(25)+kd2*J*y(20)+rho3*L4*J^4/(J^4+epsilon^4)*y(24)+L5*y(28)-(w2*k5+ku2+L3+L6*J^4/(J^4+mu^4))*y(26);%QD6
		dy(27)=s3*k4*A*y(14)+w3*k5*y(28)+kd3*J*y(21)+L6*J^4/(J^4+mu^4)*y(25)-(w3*k3+s3*k6*A+ku3+L5)*y(27);%QD7
		dy(28)=s3*k6*A*y(27)+kd3*J*y(22)+L6*J^4/(J^4+mu^4)*y(26)-(w3*k5+ku3+L5)*y(28);%QD8
