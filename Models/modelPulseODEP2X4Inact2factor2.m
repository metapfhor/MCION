function ode = modelPulseODEP2X4Inact2factor2(ton,toff)
global k1 k2 k3 k4 k5 k6 L1 L2 L3 L4 L5 L6 W1 W2 H0 H11 H5 kd1 kd2 kd3 kd4 ks1 ks2 ks3 ks4 H3 H3D H4 g1 g2 E1 E2 delta epsilon mu A J V n1 n2 n3 r1 r2 r3 v1 v2 v3 A0 J0 IVMon IVMoff;
	...
	function dy=model(t,y)
		...
		A=ATP(t);
		J=IVM(t);
		setAuxiliaryP2X4Inact2factor2(y);
		dy(1)=H5*y(2)+W1*y(4)-(H0+W2*J^4/(J^4+delta^4))*y(1);%C0
		dy(2)=H0*y(1)+k1*y(3)+L1*y(4)+H11*y(18)+H4*y(34)-(H5+k2*A+L2*J^4/(J^4+delta^4))*y(2);%C1
		dy(3)=k2*A*y(2)+k3*y(10)+ks1*y(19)+L1*y(5)-(k1+k4*A+kd1+L2*J^4/(J^4+delta^4))*y(3);%C2
		dy(4)=v1*k1*y(5)+W2*J^4/(J^4+delta^4)*y(1)+L2*J^4/(J^4+delta^4)*y(2)+L3*y(6)+ks2*y(22)-(r1*k2*A+W1+L1+L4*J^4/(J^4+epsilon^4))*y(4);%C3
		...
		dy(7)=r2*k2*A*y(6)+v2*k1*y(14)+ks3*y(27)+L4*J^4/(J^4+epsilon^4)*y(5)+L5*y(9)-(v2*k1+r2*k4*A+kd3+L3+L6*J^4/(J^4+mu^4))*y(7);%C6
		...
		dy(14)=r2*k4*A*y(7)+v2*k1*y(15)+ks3*y(28)+L4*J^4/(J^4+epsilon^4)*y(12)+L5*y(16)-(v2*k1+r2*k6*A+kd3+L3+L6*J^4/(J^4+mu^4))*y(14);%Q5
		dy(15)=r2*k6*A*y(14)+ks3*y(29)+L4*J^4/(J^4+epsilon^4)*y(13)+L5*y(17)-(v2*k1+kd3+L3+L6*J^4/(J^4+mu^4))*y(15);%Q6
		...
		dy(18)=k1*y(19)+L1*y(22)-(k2*A+L2*J^4/(J^4+delta^4)+H11)*y(18);%D11
