function ode = modelODEanmarP2X4sense(ton,toff)
global k1 k2 k3 k4 k5 k6 L1 L2 L3 H1 H2 H3 H4 g1 g2 E1 E2 alpha beta J K F G A IVMon IVMoff;
	...
	function dy=model(t,y)
		setAuxiliaryanmarP2X4sense(y);
		dy=zeros(13,1);
		A=ATP(t);
		J=IVM(t);
		dy(1)=k1*G*y(2)+L1*y(3)+H1*y(9)+H4*y(13)-3*k2*A*y(1);%C1
		dy(2)=3*k2*A*y(1)+2*k3*G*y(5)-(k1*G+2*k4*A+H2)*y(2);%C2
		dy(3)=k1*G*y(4)-(3*k2*A*F+L1)*y(3);%C3
		dy(4)=3*k2*A*F*y(3)+2*k1*G*y(7)-(k1*G+2*k2*A*F)*y(4);%C4
		dy(5)=2*k4*A*y(2)+3*k5*G*y(6)-(2*k3*G+k6*A+H2)*y(5);%Q1
		dy(6)=k6*A*y(5)+L2*y(8)-(3*k5*G+H2+L3*(1-G))*y(6);%Q2
